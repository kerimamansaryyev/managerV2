...
void main() {
  ...
  test(
    ...
    manager.dispose();
    expect(manager.observers.length, 0);
    expect(test_manager_observer.TestCounterManager.onDisposeCalled, 2);
  });
  ...
  test(
    ...
    manager.run(AsynchronousTask.generic(
        ...
        computation: () =>
            Future.delayed(const Duration(seconds: 2), () => 3)));
    await manager.waitForTaskToBeDone(taskId: 'one');
    expect(test_manager_observer.TestCounterManager.observer0.hasStateMutated,
        true);
    expect(test_manager_observer.TestCounterManager.observer1.hasStateMutated,
        true);
    test_manager_observer.TestCounterManager.observer0.tearDown();
    test_manager_observer.TestCounterManager.observer1.tearDown();
    manager.run(AsynchronousTask.generic(
